...
public class LockTest {
    ...
    public LockTest(){
        try {
            zk = new ZooKeeper(zkQurom, 6000,
                    /*n -> {
                        System.out.println(Thread.currentThread().getName() + "Receive event "+n);
                        if(Watcher.Event.KeeperState.SyncConnected == n.getState()){
            ...
            new Watcher() {
                ...
                public void process(WatchedEvent watchedEvent) {
                    System.out.println(Thread.currentThread().getName() + "Receive event =c="+watchedEvent);
                    if(Event.KeeperState.SyncConnected == watchedEvent.getState())
                        System.out.println(Thread.currentThread().getName() + " =c= connection is established...");
                }
            }
            );
        } catch (IOException e) {
    ...
    private void watchNode(String nodeString, final Thread thread) throws InterruptedException {
        try {
            zk.exists(nodeString, new Watcher() {
                @Override
                public void process(WatchedEvent watchedEvent) {
                    System.out.println(Thread.currentThread().getName() +  "=a=" + watchedEvent.toString());
                    if(watchedEvent.getType() == Event.EventType.NodeDeleted){
                        System.out.println(Thread.currentThread().getName() + "Threre is a Thread released Lock=a=============");
                        thread.interrupt();
                    }
                    try {
                        zk.exists(nodeString,new Watcher() {
                            @Override
                            public void process(WatchedEvent watchedEvent) {
                                System.out.println(Thread.currentThread().getName() +  "=b=" + watchedEvent.toString());
                                if(watchedEvent.getType() == Event.EventType.NodeDeleted){
                                    System.out.println(Thread.currentThread().getName() + "Threre is a Thread released Lock=b=============");
                                    thread.interrupt();
                                }
                                try {
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    System.out.println(Thread.currentThread().getName() + "thread is notify " +ex.getMessage());
                }
            }
